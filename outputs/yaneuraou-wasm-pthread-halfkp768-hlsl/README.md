# @ultemica/yaneuraou-wasm-pthread-halfkp768-hlsl
...
- エンジン: YaneuraOu NNUE `HalfKP_768x2_16_64` (AobaNNUE 互換ネットワーク構造)
- スレッド: 可変 (`Threads` USI option、最大 32)
- WASM SIMD: 有効
- メモリ: 256MB initial / 2GB max / 2MB stack
- 評価関数は同梱せず外部ロード ([AobaNNUE Release](https://github.com/yssaya/AobaNNUE/releases) の `nn.bin` を想定)
- **`SharedArrayBuffer` を要求 — ホストは COOP/COEP ヘッダを返す必要**

## Sibling packages

| | `yaneuraou-wasm-cfworkers` | `yaneuraou-wasm-pthread-halfkp768` (this) |
|---|---|---|
| Engine | NNUE KP256 (Suisho petite) | NNUE HalfKP768 (AobaNNUE) |
...
```ts
import { createEngine } from "@ultemica/yaneuraou-wasm-pthread-halfkp768-hlsl";
import YaneuraOuFactory from "@ultemica/yaneuraou-wasm-pthread-halfkp768-hlsl/engine";
import wasmUrl from "@ultemica/yaneuraou-wasm-pthread-halfkp768-hlsl/wasm?url";
