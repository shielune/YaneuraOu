...
def build_body(packages: list[dict]) -> str:
    ...
    intro = (
        ...
        f"\n"
        f"**What's new in this release**: {n_node} brand-new "
        f"**Node.js-targeted** packages (`@ultemica/yaneuraou-wasm-node-*`) "
        f"join the existing {n_non_node}. Total: **{total} npm-shaped "
        f"packages**, each shipped as its own `tar.gz` asset on this page.\n"
        f"\n"
