 ...
 *   "@ultemica/yaneuraou-wasm-node-material/engine",
 * );
...
export interface CreateEngineOptions {
  /**
   * The default export from `@ultemica/yaneuraou-wasm-node-material/engine`.
   * This is the Emscripten module factory.
   */
  factory: YaneuraOuFactory;
  /**
   * Absolute path to `yaneuraou.js` (resolved via
   * `require.resolve("@ultemica/yaneuraou-wasm-node-material/engine")` in your
...
export async function createEngine(opts: CreateEngineOptions): Promise<Engine> {
  ...
  if (!fs.existsSync(shimPath)) {
    throw new Error(
      `yaneuraou-wasm-node-material: worker_shim.js not found at ${shimPath}. ` +
  ...
  if (typeof instance.callMain === "function") {
    ...
    throw new Error(
      "yaneuraou-wasm-node-material: engine.callMain is not exposed — the " +
