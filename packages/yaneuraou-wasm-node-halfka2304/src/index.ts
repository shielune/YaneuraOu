 ...
 *
 * YaneuraOu SFNN `HalfKA_hm2 2304x2-15-64` (NAGISA_V4 compatible network)
 * shogi engine for Node.js, built with
 ...
 *
 * Sister package:
 *   - `@ultemica/yaneuraou-wasm-pthread-halfka2304` — browser pthread variant
 *
 ...
 *   "@ultemica/yaneuraou-wasm-node-halfka2304/engine",
 * );
...
export interface CreateEngineOptions {
	/**
	 * The default export from `@ultemica/yaneuraou-wasm-node-halfka2304/engine`.
	 * This is the Emscripten module factory.
	 */
	factory: YaneuraOuFactory;
	/**
	 * Absolute path to `yaneuraou.js` (resolved via
	 * `require.resolve("@ultemica/yaneuraou-wasm-node-halfka2304/engine")` in your
	...
	evalFile?: string;
	/**
	 * Progress coefficients (`progress.bin`). Required for this network — the
	 * layer-stack bucket is chosen from these, and they ship separately from
	 * `nn.bin` rather than embedded in it.
	 */
	progressBin?: ArrayBuffer | Uint8Array;
	...
	class NodeWorkerPolyfill {
		...
		constructor(target: unknown, _opts: Record<string, unknown> = {}) {
			...
			if (isClassicWorker) {
				...
				this._w = new NodeWorker(target instanceof URL ? target : filepath);
			}
...
export async function createEngine(opts: CreateEngineOptions): Promise<Engine> {
	...
	if (!fs.existsSync(shimPath)) {
		throw new Error(
			`yaneuraou-wasm-node-halfka2304: worker_shim.js not found at ${shimPath}. ` +
	...
	if (typeof instance.callMain === "function") {
		...
		throw new Error(
			"yaneuraou-wasm-node-halfka2304: engine.callMain is not exposed — the " +
	...
	if (opts.progressBin) {
		instance.FS.mkdirTree("/eval");
		instance.FS.writeFile("/eval/progress.bin", toBytes(opts.progressBin));
	}
...
function parseGoResult(goLines: string[]): EvalResult {
	...
	const bm = bestmoveLine.match(/^bestmove\s+(\S+)(?:\s+ponder\s+(\S+))?/);
