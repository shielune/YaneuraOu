 ...
 * `@ultemica/yaneuraou-wasm-cfworkers` there.
 *
...
export async function createEngine(opts: CreateEngineOptions): Promise<Engine> {
	if (typeof SharedArrayBuffer === "undefined") {
		throw new Error(
			"yaneuraou-wasm-pthread-halfka2304 requires SharedArrayBuffer. Serve the page " +
	...
	if (opts.progressBin) {
		...
		instance.FS.writeFile(
			"/eval/progress.bin",
			new Uint8Array(opts.progressBin),
		);
	}
