# @ultemica/yaneuraou-wasm-pthread-halfka2304

YaneuraOu SFNN HalfKA_hm2 将棋エンジンを multi-thread (pthread) で動かすための
ブラウザ向け WASM パッケージ。

- エンジン: YaneuraOu SFNN `HalfKA_hm2 2304x2-15-64` / LayerStack 9 (NAGISA_V4 互換)
- スレッド: 可変 (`Threads` USI option、最大 32)
- WASM SIMD: 有効
- メモリ: 512MB initial / 4GB max / 2MB stack
- 評価関数は同梱せず外部ロード (NAGISA_V4 配布物の `nn.bin` と `progress.bin`)
- **`SharedArrayBuffer` を要求 — ホストは COOP/COEP ヘッダを返す必要**

## Sibling packages

| | `yaneuraou-wasm-cfworkers` | `yaneuraou-wasm-pthread-halfka2304` (this) |
|---|---|---|
| Engine | NNUE KP256 (Suisho petite) | SFNN HalfKA_hm2 (NAGISA_V4) |
...
| WASM heap | 64MB / 128MB max | 512MB / 4GB max |
...
```ts
import { createEngine } from "@ultemica/yaneuraou-wasm-pthread-halfka2304";
import YaneuraOuFactory from "@ultemica/yaneuraou-wasm-pthread-halfka2304/engine";
import wasmUrl from "@ultemica/yaneuraou-wasm-pthread-halfka2304/wasm?url";
...
|---|---|---|---|
| NAGISA_V4 nn.bin | SFNN HalfKA_hm2 2304x2-15-64 | ~172 MB (180,556,853 bytes) | NAGISA_V4 配布物のアーカイブに同梱 |
| NAGISA_V4 progress.bin | (同上) | ~1 MB (1,003,104 bytes) | **必須。** LayerStack 選択に使う進行度係数 |
...
> 計算できない。単体配布は無く、各プラットフォーム版アーカイブに `eval/`
> として同梱されている。
