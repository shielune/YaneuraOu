 ...
 *
 * YaneuraOu NNUE `NAGISA V3.1 SFNN HalfKA_hm2_1024x2_15_64` (NAGISA V3.1 compatible network)
...
export interface CreateEngineOptions {
	...
	evalFile?: string;
	/** Additional evaluation files written under `/eval` before `isready`. */
	evalFiles?: ReadonlyArray<{ name: string; data: ArrayBuffer }>;
...
export async function createEngine(opts: CreateEngineOptions): Promise<Engine> {
	...
	if (opts.evalBin || opts.evalFiles) {
		...
		for (const file of opts.evalFiles ?? []) {
			if (!/^[A-Za-z0-9_.-]+$/.test(file.name)) {
				throw new Error(`invalid evaluation file name: ${file.name}`);
			}
			instance.FS.writeFile(`/eval/${file.name}`, new Uint8Array(file.data));
		}
	}
