# yaneuraou-wasm-pthread-halfkp128 — internal spec for LLM consumers
...
- npm scope: `@ultemica/yaneuraou-wasm-pthread-halfkp128`
- Engine: YaneuraOu NNUE HalfKP128 V9.60, multi-threaded
...
- Memory ceiling is 1 GB (vs 128 MB on cfworkers). HalfKP eval still does
  not fit a typical HalfKP128 engine binary, but `USI_Hash` can be raised
...
Hard limits:
- Eval: HalfKP128 (~31 MB) only.
- 700T-shock book + 256 MB hash + 16 threads stays under 1 GB.

## Error Handling

- Eval file missing: `exit(1)` during `isready`.
- Wrong eval architecture (KP256 into HalfKP128 engine): `exit(1)` during `isready`.
