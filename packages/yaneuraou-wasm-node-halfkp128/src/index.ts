 ...
 * import { createEngine } from "@ultemica/yaneuraou-wasm-node-halfkp128";
 * import YaneuraOuFactory from "@ultemica/yaneuraou-wasm-node-halfkp128/engine";
 *
 * const require = createRequire(import.meta.url);
 * const wasmPath = require.resolve(
 *   "@ultemica/yaneuraou-wasm-node-halfkp128/wasm",
 * );
 * const enginePath = require.resolve(
 *   "@ultemica/yaneuraou-wasm-node-halfkp128/engine",
 * );
...
export interface CreateEngineOptions {
  /**
   * The default export from `@ultemica/yaneuraou-wasm-node-halfkp128/engine`.
   * This is the Emscripten module factory.
   */
  factory: YaneuraOuFactory;
  /**
   * Absolute path to `yaneuraou.js` (resolved via
   * `require.resolve("@ultemica/yaneuraou-wasm-node-halfkp128/engine")` in your
...
export async function createEngine(opts: CreateEngineOptions): Promise<Engine> {
  ...
  if (!fs.existsSync(shimPath)) {
    throw new Error(
      `yaneuraou-wasm-node-halfkp128: worker_shim.js not found at ${shimPath}. ` +
  ...
  if (typeof instance.callMain === "function") {
    ...
    throw new Error(
      "yaneuraou-wasm-node-halfkp128: engine.callMain is not exposed — the " +
