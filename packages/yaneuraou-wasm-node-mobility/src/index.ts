 ...
 *   "@ultemica/yaneuraou-wasm-node-mobility/engine",
 * );
...
export interface CreateEngineOptions {
  /**
   * The default export from `@ultemica/yaneuraou-wasm-node-mobility/engine`.
   * This is the Emscripten module factory.
   */
  factory: YaneuraOuFactory;
  /**
   * Absolute path to `yaneuraou.js` (resolved via
   * `require.resolve("@ultemica/yaneuraou-wasm-node-mobility/engine")` in your
...
export async function createEngine(opts: CreateEngineOptions): Promise<Engine> {
  ...
  if (!fs.existsSync(shimPath)) {
    throw new Error(
      `yaneuraou-wasm-node-mobility: worker_shim.js not found at ${shimPath}. ` +
  ...
  if (typeof instance.callMain === "function") {
    ...
    throw new Error(
      "yaneuraou-wasm-node-mobility: engine.callMain is not exposed — the " +
