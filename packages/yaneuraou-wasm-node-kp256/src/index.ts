 ...
 *   "@ultemica/yaneuraou-wasm-node-kp256/wasm",
 * );
 * const enginePath = require.resolve(
 *   "@ultemica/yaneuraou-wasm-node-kp256/engine",
 * );
...
export interface CreateEngineOptions {
  /**
   * The default export from `@ultemica/yaneuraou-wasm-node-kp256/engine`.
   * This is the Emscripten module factory.
   */
  factory: YaneuraOuFactory;
  /**
   * Absolute path to `yaneuraou.js` (resolved via
   * `require.resolve("@ultemica/yaneuraou-wasm-node-kp256/engine")` in your
...
export async function createEngine(opts: CreateEngineOptions): Promise<Engine> {
  ...
  if (!fs.existsSync(shimPath)) {
    throw new Error(
      `yaneuraou-wasm-node-kp256: worker_shim.js not found at ${shimPath}. ` +
  ...
  if (typeof instance.callMain === "function") {
    ...
    throw new Error(
      "yaneuraou-wasm-node-kp256: engine.callMain is not exposed — the " +
