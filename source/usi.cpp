...
void USIEngine::UnitTest(Test::UnitTester& tester, IEngine& engine) {
...

    {
        ...
        if (!options.count(hidden_option_name))
            options.add(hidden_option_name, Option(false).hidden());
        ...
        tester.test("hidden option is not advertised",
                    output.str().find("option name UnitTestHiddenOption type ") == std::string::npos);

        std::istringstream set_hidden("name UnitTestHiddenOption value true");
        options.setoption(set_hidden);
        tester.test("hidden option accepts setoption", bool(options[hidden_option_name]));

        std::istringstream reset_hidden("name UnitTestHiddenOption value false");
        options.setoption(reset_hidden);
    }
