...
- npm scope: `@ultemica/yaneuraou-wasm-material-cfworkers-hlsl`
- Engine: YaneuraOu Material Lv1 V8.50 (`YANEURAOU_ENGINE_MATERIAL` with `MATERIAL_LEVEL=1`)
- Runtime: Cloudflare Workers (V8 Isolate, single-thread, WASM SIMD)
- License: GPL-3.0
- Eval data: embedded (handcrafted material-only evaluator, no external file required)
...
```ts
import { createEngine } from "@ultemica/yaneuraou-wasm-material-cfworkers-hlsl";
