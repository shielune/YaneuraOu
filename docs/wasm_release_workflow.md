...
既存の sibling パッケージをまるごとコピーして名前を変える。例えば
`yaneuraou-wasm-node-kp256` を雛形にして `yaneuraou-wasm-node-foo` を作るなら:

```sh
cp -R yaneuraou-wasm-node-kp256 yaneuraou-wasm-node-foo
...
| `dir` | パッケージディレクトリ名 | `yaneuraou-wasm-node-foo` |
...
```sh
git add yaneuraou-wasm-node-foo .github/workflows/build-wasm.yml
