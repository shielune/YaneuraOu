...
- **New: HalfKP_128x2_32_32** (`pthread-halfkp128`, `node-halfkp128`). Half the
  ...
  Workers heap.
- **New: the NAGISA_V3 network is supported** (`pthread-halfka1024`, `node-halfka1024`).
...
- **`FV_SCALE` now defaults to 28 on SFNN builds** (16 elsewhere). Together with
  ...
  `halfka1024-cfworkers` variant: the 75 MB eval does not fit the 128 MB Workers heap.
- **Heap ceiling raised to 4 GB** on every pthread and Node package (the wasm32
  maximum), up from 1–2 GB. `MAXIMUM_MEMORY` is an address-space reservation, not
...
- **`USI_Hash`'s ceiling is now derived from the heap** rather than hard-coded to
  ...
  build would happily accept `USI_Hash 2048` and then die allocating it; now the
...
- **Fixed: NNUE evaluation was silently wrong on plain-NEON builds** — the
  ...
  `USE_NEON >= 8`, but only reads that layout back when `USE_NEON_DOTPROD` is
  set; without dotprod it falls back to the dense kernel, so the load and the
  compute disagreed. The plain-NEON kernels also treated the `uint8` input as
