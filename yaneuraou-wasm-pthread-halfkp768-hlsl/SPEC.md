# yaneuraou-wasm-pthread-halfkp768 — internal spec for LLM consumers
...
- npm scope: `@ultemica/yaneuraou-wasm-pthread-halfkp768-hlsl`
- Engine: YaneuraOu NNUE HalfKP768 V8.50, multi-threaded
...
- Source: `source/` (engine: `YANEURAOU_ENGINE_NNUE_HALFKP_768X2_16_64`)
- NNUE architecture: `HalfKP_768x2_16_64` (AobaNNUE 互換)
- Linker flags (em++):
  - `EM_EXPORT_NAME=YaneuraOu_HalfKP768`
...
- Memory ceiling is 2 GB (vs 128 MB on cfworkers). `USI_Hash` can be raised
  significantly (64–512 MB) along with the AobaNNUE evaluator footprint.
- NNUE eval must follow the `HalfKP_768x2_16_64` layout — Suisho5 系の
  `HalfKP_256x2_32_32` 重みや KP256 重みは互換性なし。AobaNNUE 公式 Release
  の `nn.bin` を `EvalDir` 配下に書き出して使う。
...
Hard limits:
- Eval: HalfKP768 (~60 MB) only.
- 700T-shock book + 256 MB hash + 16 threads stays under 1 GB.

## Error Handling

- Eval file missing: `exit(1)` during `isready`.
- Wrong eval architecture (KP256 into HalfKP768 engine): `exit(1)` during `isready`.
